...
\begin{MyTh}{Fenichel's Theorem}
    ...
    \quad {\scriptsize \ok{Find $\mathcal{M}_\eps$}: If $\mathcal{M}_0=\{(u,v):u=p_0(v)\}$, then $\mathcal{M}_\eps=\{(u,v):u=p_{\eps}(v)\}$ \quad \scalebox{0.9}{\ps{\tiny 可通过算$u,v$的exact sol, 寻找能使含$t$项消失但保留$\tau$的解(or:考虑快变量$t\to\infty$), 对比$\mathcal{M}_0$得到$\mathcal{M}_\eps$}}} \qquad\quad {\tiny $\downarrow$ \ok{Infective Period}: =$\frac{1}{\alpha}$}
\end{MyTh}
