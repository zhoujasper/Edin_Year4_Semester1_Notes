...
\begin{MyTh}{假设和默认}
    \tiny $^1$ 若要假设$u(\mathbf{x},t)$的IC,考虑 $u(\mathbf{x},0)=b_0>0$ \scalebox{0.85}{\ps{\tiny (均匀的分布)}} \ \ $^2$ \ok{Stationary Sol}: For $u(\mathbf{x},t)$, Let $u_t=0$ to find sol. \ \ $^3$ \ok{Homogeneous Rest State}: For $u(\mathbf{x},t)$, Let $u_{\mathbf{x}}=0$ and $u_t=0$ to find sol.
\end{MyTh}
