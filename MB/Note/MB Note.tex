...
\begin{MyTh}{Remark for Boundary}
    \tiny $^1$ 任何 \ps{\tiny Diffusion} Model: 认为 $u(\mathbf{x},t)$ 是 \ok{bounded} 的. \scalebox{0.85}{\ps{\tiny (含$\pm\infty$, 极坐标$r\to\infty$, 得IC)}} \ \ $^2$ 对 $u$, $J$ \scalebox{0.85}{(flux)}, 有 \ok{continuous}. \scalebox{0.85}{\ps{\tiny (得BC,尤其是分段函数)}} \ \ $^3$ 对 $J$ \scalebox{0.85}{(flux)}, 考虑由物理意义, 边界\ps{\tiny 是否为0}
\end{MyTh}

\begin{MyTh}{假设和默认}
    \tiny $^1$ 若要假设$u(\mathbf{x},t)$的IC,考虑 $u(\mathbf{x},0)=b_0>0$ \scalebox{0.85}{\ps{\tiny (均匀的分布)}} \ \ $^2$ \ok{Stationary Sol}: For $u(\mathbf{x},t)$, Let $u_t=0$ to find sol.
\end{MyTh}

\begin{MyTh}{Useful Tools}
    \tiny $^1$\ok{最后,可通过两边对$\mathbf{u}$对$\mathbf{x}$积分得待定系数值} \ \ $^2$ \ok{Linearise at $u^*$}: Let $u=u^*+\eta$, $|\eta|\ll1$ func. \ \ $^3$ 对于求解PDE, 考虑\ps{\tiny 直接计算} or \ps{\tiny Seperation of var.}
\end{MyTh}
